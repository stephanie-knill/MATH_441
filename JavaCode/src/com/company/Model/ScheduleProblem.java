...
public class ScheduleProblem {
    ...
    public int problemValue() {
        ...
        for (int i = 1; i < length; i++) {
            int deltax = currentSchedule.get(i - 1).getX() -
                    currentSchedule.get(i).getX();
            int deltay = currentSchedule.get(i - 1).getY() -
                    currentSchedule.get(i).getY();
            distance = distance + Math.abs(deltax) + Math.abs(deltay);
        }
        return distance;
    }
